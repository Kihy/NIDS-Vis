...
class FE:
    def __init__(self, file_path, parse_type, limit=None, nstat=None, dummy_nstat=None, force_tsv=True, log_file=None, offset=0):
        self.path = file_path
        self.limit = limit
        ...
        if nstat is not None:
            ...
            self.nstat = ns.netStat(
                np.nan, self.maxHost, self.maxSess, log_path=log_file)
    ...
    def pcap2tsv_with_tshark(self):
        ...
        if self.limit is not None:
            ...
            options = ""
        cmd = 'tshark -r ' + self.path + ' -T fields ' + fields + \
            options + ' -E header=y -E occurrence=f > '+self.path+".tsv"
    ...
    def get_next_vector_scapy(self):
        packet = self.input_stream.read_packet()
        ...
        # print(dir(packet))
        # add offset to time
        timestamp = packet.time + self.offset
        framelen = len(packet)

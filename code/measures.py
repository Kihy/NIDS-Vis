...
class MeasureCalculator():
    ...
    def _trustworthiness(x_neighbourhood, x_ranks, z_neighbourhood,
        ...
        for row in range(x_ranks.shape[0]):
            missing_neighbours = np.setdiff1d(
                z_neighbourhood[row],
                x_neighbourhood[row], assume_unique=True
            )
            trust += np.sum(x_ranks[row][missing_neighbours] - k)

        return 1 - 2 / (n * k * (2 * n - 3 * k - 1)) * trust
    ...
    def mrre(self, k):
        ...
        rx = np.take_along_axis(x_ranks, z_neighbourhood, axis=1)
        rz = np.take_along_axis(z_ranks, z_neighbourhood, axis=1)
        mrre_ZX = np.sum(np.abs(rx - rz) / rz)

        # Second component goes from the data space to the latent space,
        # i.e. the relative quality of neighbours in `X`.

        rx = np.take_along_axis(x_ranks, x_neighbourhood, axis=1)
        rz = np.take_along_axis(z_ranks, x_neighbourhood, axis=1)
        mrre_XZ = np.sum(np.abs(rx - rz) / rx)

        # Normalisation constant
        C = n * sum([abs(n - 2 * j + 1) / j for j in range(1, k + 1)])
